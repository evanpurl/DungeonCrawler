...
async def enterdungeon(ctx, bot, user, id, character):
    def is_auth(m):
    ...
    async def finishcallback(interaction):  # Finish Button
        ...
        if not os.path.exists(f"{dirr}/Players/{str(id)}/class.txt"):
            if int(player.level) >= 10:
                ...
                if int(choice.content) in classnum:
                    ...
                    with open(f"{dirr}/Players/{str(id)}/class.txt", "w+") as pclass:
                    ...
                    with open(f"{dirr}/Players/{str(id)}/damage.txt", "r") as dam:
                        d = int(dam.readline())
                    with open(f"{dirr}/Players/{str(id)}/damage.txt", "w+") as dam:
                        dam.write(str(d + int(damage)))
                    with open(f"{dirr}/Players/{str(id)}/health.txt", "r") as hea:
                        h = int(hea.readline())
                    with open(f"{dirr}/Players/{str(id)}/health.txt", "w+") as hea:
                        hea.write(str(h + int(health)))
                    with open(f"{dirr}/Players/{str(id)}/defense.txt", "r") as dam:
                        d = int(dam.readline())
                    with open(f"{dirr}/Players/{str(id)}/defense.txt", "w+") as dam:
                        dam.write(str(d + int(deff)))
                else:
                    await user.send(
                        f"The option you chose is not available, finish Dungeon Crawler to get this prompt again.")
            ...
            if enmove == "attack":
                ...
                if damage <= 0:
                    ...
                    if player.health <= 0:
                        if room == maxrooms:
                            await user.send(
                                f"You have been killed by the {enemyclass.getname()}. Ending Dungeon Crawler.")
                        else:
